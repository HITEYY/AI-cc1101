 ...
 public:
  ...
  void renderBase(const String &title,
    ...
    if (subtitle.length() > 0) {
      ...
      lv_obj_align(subLabel, LV_ALIGN_LEFT_MID, 4, 0);
      y += kSubtitleHeight;
    }

    int footerY = h - kFooterHeight;
    if (statusLine.length() > 0) {
      footerY -= kStatusHeight;
      lv_obj_t *statusBar = lv_obj_create(screen);
      lv_obj_remove_style_all(statusBar);
      lv_obj_set_pos(statusBar, 0, footerY);
      lv_obj_set_size(statusBar, w, kStatusHeight);
      lv_obj_set_style_bg_color(statusBar, lv_color_hex(0x232323), 0);

      lv_obj_t *statusText = lv_label_create(statusBar);
      setLabelFont(statusText);
      lv_label_set_text(statusText, statusLine.c_str());
      lv_obj_set_style_text_color(statusText, lv_color_white(), 0);
      lv_obj_align(statusText, LV_ALIGN_LEFT_MID, 2, 0);
    }
  ...
  void renderMenu(const String &title,
    ...
    const int w = lv_display_get_horizontal_resolution(port.display());
    int maxRows = (contentBottom - contentTop) / kRowHeight;
    if (maxRows < 1) {
      maxRows = 1;
    }
    ...
    for (int row = 0; row < maxRows; ++row) {
      ...
      lv_obj_set_size(btn, w, kRowHeight - 1);
  ...
  void renderInfo(const String &title,
    ...
    for (int row = 0; row < maxRows; ++row) {
      ...
      lv_obj_set_pos(holder, 0, y);
      lv_obj_set_size(holder, w, kRowHeight - 1);
  ...
  void renderToast(const String &title,
    ...
    const int w = lv_display_get_horizontal_resolution(port.display());
    const int h = contentBottom - contentTop;

    lv_obj_t *box = lv_obj_create(lv_screen_active());
    lv_obj_set_size(box, w - 16, h > 40 ? h - 12 : h);
    lv_obj_center(box);
  ...
  void renderProgressOverlay(const String &title,
    ...
    if (needsCreate) {
      ...
      lv_obj_set_size(progressPanel, w - 20, h > 140 ? h - 84 : h - 28);
      ...
      setLabelFont(progressTitle);
      lv_obj_set_style_text_color(progressTitle, lv_color_white(), 0);
      lv_obj_align(progressTitle, LV_ALIGN_TOP_LEFT, 10, 8);

      progressSpinner = lv_spinner_create(progressPanel);
      lv_obj_set_size(progressSpinner, 22, 22);
      lv_obj_align(progressSpinner, LV_ALIGN_TOP_RIGHT, -10, 6);

      progressMessage = lv_label_create(progressPanel);
      setLabelFont(progressMessage);
      lv_label_set_long_mode(progressMessage, LV_LABEL_LONG_WRAP);
      lv_obj_set_width(progressMessage, w - 44);
      lv_obj_set_style_text_color(progressMessage, lv_color_white(), 0);
      lv_obj_align(progressMessage, LV_ALIGN_CENTER, 0, -6);

      progressBar = lv_bar_create(progressPanel);
      lv_obj_set_size(progressBar, w - 48, 10);
      lv_obj_align(progressBar, LV_ALIGN_BOTTOM_MID, 0, -18);
      ...
      setLabelFont(progressPercent);
      lv_obj_set_style_text_color(progressPercent, lv_color_hex(0xA5E8FF), 0);
      lv_obj_align(progressPercent, LV_ALIGN_BOTTOM_RIGHT, -12, -32);
    }
...
bool UiRuntime::textInput(const String &title,
  ...
  const int keyGap = 2;
  const int keyboardTop = kHeaderHeight + kSubtitleHeight + 4;
  ...
  auto addCharRow = [&](const CharKeyPair *row, size_t len, int y) {
    ...
    int x = (lv_display_get_horizontal_resolution(impl_->port.display()) - rowWidth) / 2;
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    slot.area.x2 = actionX + kActionWidths[i] - 1;
